...
def _fresh_record(at) -> CachedSessionRecord:
...
def _owner(cache=None, clock=None):
    ...
    session = SharedCachedSession(
        inner=inner, cache=cache, clock=clock, refresh_ttl_seconds=_TTL, owner=True
    )
...
def _reader(cache=None, clock=None):
    ...
    inner = _SpySession()
    session = SharedCachedSession(
        inner=inner, cache=cache, clock=clock, refresh_ttl_seconds=_TTL, owner=False
    )
    return session, inner, cache, clock


def test_owner_authenticates_and_writes_cache_when_empty():
...
def test_owner_does_not_reauth_within_ttl():
    clock = FakeClock(_SEED)
    session, inner, _, _ = _owner(clock=clock)
...
def test_owner_reauths_and_rewrites_after_ttl():
    clock = FakeClock(_SEED)
...
def test_reader_never_calls_inner_authenticate():
    cache = FakeSessionCache(_fresh_record(_SEED))
...
def test_reader_waits_silently_until_token_appears():
    ...
    def load_then_populate():
        calls["n"] += 1
        if calls["n"] >= 3:
            cache._record = _fresh_record(clock.utcnow())
...
def test_reader_treats_expired_cache_as_absent_and_waits():
    stale = _fresh_record(_SEED)
    clock = FakeClock(_SEED)
    clock.advance(_TTL + 1.0)
    cache = FakeSessionCache(stale)
    session, inner, _, _ = _reader(cache=cache, clock=clock)

    calls = {"n": 0}

    def load_then_refresh():
        calls["n"] += 1
        if calls["n"] >= 2:
            cache._record = _fresh_record(clock.utcnow())
        return cache._record

    cache.load = load_then_refresh
